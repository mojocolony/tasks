(() => {
  ...
  function clearCompleted(){
    ...
    const ts=now();
    completed.forEach(t=>state.deletedTasks[t.id]=ts);
    state.tasks=state.tasks.filter(t=>!t.completed);
    saveState();
    toast(`${completed.length} completed task${completed.length===1?'':'s'} deleted`);
  }
